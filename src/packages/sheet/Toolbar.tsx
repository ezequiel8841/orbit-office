...
export function Toolbar({ ctrl }: { ctrl: SheetController }) {
  ...
  const TABS = [
    ...
    { id: "data",  label: t("tab.data") },
    { id: "tools", label: t("tab.tools") },
    { id: "file",  label: t("tab.file") },
  ];

  return (
    <div role="toolbar" aria-label="Spreadsheet toolbar">
      {/* Tab bar */}
      ...
      <div className="oo-ribbon-pane">
        {ribbonTab === "home" && <>
          ...
          <ToolGrp label={t("grp.history")}>
            <button className="oo-btn" aria-label="Undo"
              onClick={() => { ctrl.history.undo(); ctrl.store.set((s) => ({ ...s })); }}
              disabled={!ctrl.history.canUndo()}>
              <IconUndo />{t("lbl.undo")}
            </button>
            <button className="oo-btn" aria-label="Redo"
              onClick={() => { ctrl.history.redo(); ctrl.store.set((s) => ({ ...s })); }}
          ...
          <ToolGrp label={t("grp.font")}>
            <select className="oo-select" aria-label="Font family" style={{ minWidth: 100 }}
            ...
            <input className="oo-input" type="number" aria-label="Font size"
              style={{ width: 48 }} min={8} max={72} placeholder="13"
              value={cur.fontSize ?? ""}
              onChange={(e) => {
            ...
            <button className="oo-btn" aria-label="Wrap text" aria-pressed={!!cur.wrapText}
              title="Wrap text" onClick={() => toggle({ wrapText: !cur.wrapText })}>
          ...
          <ToolGrp label={t("grp.style")}>
            <button className="oo-btn" aria-label="Bold" aria-pressed={!!cur.bold}
              onClick={() => toggle({ bold: !cur.bold })}><IconBold /></button>
            <button className="oo-btn" aria-label="Italic" aria-pressed={!!cur.italic}
              onClick={() => toggle({ italic: !cur.italic })}><IconItalic /></button>
            <button className="oo-btn" aria-label="Underline" aria-pressed={!!cur.underline}
              onClick={() => toggle({ underline: !cur.underline })}><IconUnderline /></button>
          </ToolGrp>

          {/* ── Alinhamento ── */}
          <ToolGrp label={t("grp.alignment")}>
            <button className="oo-btn" aria-label="Align left" aria-pressed={cur.align === "left"}
              onClick={() => toggle({ align: "left" })}><IconAlignLeft /></button>
            <button className="oo-btn" aria-label="Align center" aria-pressed={cur.align === "center"}
              onClick={() => toggle({ align: "center" })}><IconAlignCenter /></button>
            <button className="oo-btn" aria-label="Align right" aria-pressed={cur.align === "right"}
              onClick={() => toggle({ align: "right" })}><IconAlignRight /></button>
          </ToolGrp>

          {/* ── Formato ── */}
          <ToolGrp label={t("grp.format")}>
            <select className="oo-select" aria-label="Number format"
        ...
        {ribbonTab === "data" && <>
          ...
          <ToolGrp label={t("grp.data")}>
            <button className="oo-btn" aria-label="Sort ascending" title={t("sheet.sortAsc")}
              onClick={() => {
                const sel = normalizeRange(state.selection);
                const range = sel.r1 === sel.r2 && sel.c1 === sel.c2
                  ? { r1: 0, c1: 0, r2: sh.numRows - 1, c2: sh.numCols - 1 } : sel;
                ctrl.exec({ kind: "sort", range, keys: [{ col: sel.c1, desc: false }] });
              }}>
              <IconSortAsc />A→Z
            </button>
            <button className="oo-btn" aria-label="Sort descending" title={t("sheet.sortDesc")}
              onClick={() => {
                const sel = normalizeRange(state.selection);
                const range = sel.r1 === sel.r2 && sel.c1 === sel.c2
                  ? { r1: 0, c1: 0, r2: sh.numRows - 1, c2: sh.numCols - 1 } : sel;
            ...
            <button className="oo-btn" aria-label="Filter column" aria-pressed={filterOpen}
              title={t("sheet.filter")} onClick={() => setFilterOpen((v) => !v)}>
          ...
          <ToolGrp label={t("grp.cells")}>
            <button className="oo-btn" aria-label="Merge cells" title={t("sheet.merge")}
              onClick={() => ctrl.exec({ kind: "merge", range: state.selection })}>
              <IconMerge />{t("lbl.merge")}
            </button>
            <button className="oo-btn" aria-label="Unmerge cells" title={t("sheet.unmerge")}
              onClick={() => ctrl.exec({ kind: "unmerge", range: state.selection })}>
              <IconUnmerge />{t("lbl.unmerge")}
            </button>
            <button className="oo-btn" aria-label="All borders" title={t("sheet.bordersAll")}
              onClick={() => ctrl.exec({ kind: "applyBorders", range: state.selection, borders: {}, mode: "all" })}>
              <IconBorderAll />{t("lbl.allBorders")}
            </button>
            <button className="oo-btn" aria-label="Outside borders" title={t("sheet.bordersOutside")}
              onClick={() => ctrl.exec({ kind: "applyBorders", range: state.selection, borders: {}, mode: "outside" })}>
              <IconBorderOutside />{t("lbl.outerBorder")}
            </button>
            <button className="oo-btn" aria-label="Clear borders" title={t("sheet.bordersNone")}
              onClick={() => ctrl.exec({ kind: "applyBorders", range: state.selection, borders: {}, mode: "clear" })}>
        ...
        {ribbonTab === "tools" && <>
          ...
          <ToolGrp label={t("grp.tools")}>
            <button className="oo-btn" aria-label="Freeze rows/cols"
              ...
              onClick={() => {
                if (sh.freezeRows > 0 || sh.freezeCols > 0) {
                  ...
                  ctrl.exec({ kind: "setFreeze", rows: state.active.row, cols: state.active.col });
                }
              }}>
              <IconFreeze />{t("lbl.freeze")}
            </button>
            <button className="oo-btn" aria-label="Data validation" aria-pressed={dvOpen}
              title={t("sheet.validation")} onClick={() => setDvOpen((v) => !v)}>
              <IconCheck />{t("lbl.validate")}
            </button>
            <button className="oo-btn" aria-label="Conditional formatting" aria-pressed={cfOpen}
              title={t("sheet.condFormat")} onClick={() => setCfOpen((v) => !v)}>
              <IconBorderAll />{t("lbl.condFormat")}
            </button>
            <button className="oo-btn" aria-label="Insert chart" title={t("sheet.chart")}
              onClick={() => {
                const sel = normalizeRange(state.selection);
                ...
                ctrl.exec({
                  ...
                  chart: {
                    ...
                    hasHeader: true, x: 200, y: 60, w: 360, h: 220, title: "Chart",
                  },
                });
              }}>
        ...
        {ribbonTab === "file" && <>
          ...
          <ToolGrp label={t("grp.export")} end>
            <button className="oo-btn" aria-label="Import CSV" onClick={importCsv}>
              <IconUpload /><span style={{ marginLeft: 6 }}>{t("common.import")} CSV</span>
            </button>
            <button className="oo-btn" aria-label="Export CSV" onClick={exportCsv}>
              <IconDownload /><span style={{ marginLeft: 6 }}>{t("common.export")} CSV</span>
            </button>
            <button className="oo-btn" aria-label="Open .xlsx" title="Open Excel file (.xlsx)"
              onClick={() => {
                const input = document.createElement("input");
                input.type = "file";
                ...
                input.onchange = async () => {
                  const file = input.files?.[0];
                  if (!file) return;
                  try { const wb = await importXlsx(file); ctrl.loadWorkbook(wb); }
                  catch (e) { console.error("XLSX import failed", e); }
                };
                input.click();
              }}>
              <IconUpload /><span style={{ marginLeft: 6 }}>Open xlsx</span>
            </button>
            <button className="oo-btn" aria-label="Save .xlsx" title="Save as Excel file (.xlsx)"
              onClick={() => {
                const filename = (sh.name || "workbook") + ".xlsx";
                exportXlsx(ctrl.store.get().workbook, filename).catch((e) =>
                  console.error("XLSX export failed", e));
              }}>
              <IconDownload /><span style={{ marginLeft: 6 }}>Save xlsx</span>
            </button>
          </ToolGrp>
        </>}
      </div>
      ...
      {filterOpen && (
        <FilterPopover ctrl={ctrl} col={state.active.col} onClose={() => setFilterOpen(false)} />
      )}
      {dvOpen && (
