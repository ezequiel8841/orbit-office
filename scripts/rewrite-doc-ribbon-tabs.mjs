...
// 1) Inject ribbonTab state after pageOrientation state line
const STATE_ANCHOR = '  const [pageOrientation, setPageOrientation] = useState<"portrait" | "landscape">("portrait");';
const STATE_INJECT = `\n  const [ribbonTab, setRibbonTab] = useState("home");`;
const code1 = code.replace(STATE_ANCHOR, STATE_ANCHOR + STATE_INJECT);

// 2) Replace the flat toolbar div with the tabbed ribbon
// Find: opening of the outer toolbar div
const OLD_TOOLBAR_START = '      <div\n        className="oo-toolbar"';
// Find: the first </div> that closes it — we know from the script it ends right before the inTable block
const OLD_TOOLBAR_END = '      </div>\n\n      {inTable &&';

const si = code1.indexOf(OLD_TOOLBAR_START);
const ei = code1.indexOf(OLD_TOOLBAR_END) + '      </div>'.length;
